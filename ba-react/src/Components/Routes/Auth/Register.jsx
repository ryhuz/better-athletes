...
function Register({ isAuth, setAuth }) {
  ...
  async function submit(user) {
    ...
    try {
      await axios.post("https://better-ath-django.herokuapp.com/api/signup", user);
      try {
  ...
  async function checkUserExists(username) {
    ...
    let exists = await axios.put("https://better-ath-django.herokuapp.com/api/user_exists", { username: username });
            ...
            <Form.Row className="my-4 text-left">
              ...
              <Col className="mx-3">
                ...
                <Form.Control onChange={handleChange} name="club" as="select" className={touched.club && errors.club ? `is-invalid` : null}>
                  <option value=" ">Select One</option>
                  {club && club.map((el,index) => (
                    <option key={index} value={el.id}>{el.club_name}</option>
                  ))}
                </Form.Control>
